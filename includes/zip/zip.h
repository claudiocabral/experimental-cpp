...
namespace Akura {
    template <class... Args>
        struct Iterator {
            ...
            constexpr inline auto operator==(const Iterator<Args...> &other) {
                return std::apply([](Args &&... a, Args &&... b) {
                        return ((a == b) || ... ); },
            ...
            constexpr inline auto operator*() {
                return std::apply([](auto &&... args) { return std::tuple(*args...); }, its);
            }
        };
    ...
    template <class T> auto enumerate(T &&t) { return Akura::Zip(std::forward<T>(t), IndexIterator{0}); }
